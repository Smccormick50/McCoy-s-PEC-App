...
function bindViewEvents(p) {
  ...
  document.getElementById('btnPrint').addEventListener('click', async (e) => {
    ...
    // Build overlay via DOM APIs only — never put blobUrl into innerHTML.
    // Embedding a 6-8MB blob URL in a template literal overflows Chrome's
    // HTML parser and dumps raw binary into the main page.
    const overlay = document.createElement('div');
    overlay.id = 'pdfOverlay';
    overlay.style.cssText = 'position:fixed;inset:0;z-index:500;background:#333;display:flex;flex-direction:column;';

    // Header
    const hdr = document.createElement('div');
    hdr.style.cssText = 'background:#0e4a2a;padding:12px 16px;display:flex;align-items:center;gap:12px;flex-shrink:0;';
    const ttl = document.createElement('span');
    ttl.style.cssText = 'color:#f5d116;font-weight:600;flex:1;font-size:14px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;';
    ttl.textContent = String.fromCodePoint(0x1F4C4) + ' ' + pdfResult.filename;
    hdr.appendChild(ttl);
    if (!isIOS) {
      const hint = document.createElement('span');
      hint.style.cssText = 'color:rgba(255,255,255,0.65);font-size:12px;white-space:nowrap;';
      hint.textContent = 'Use the save button in the PDF viewer';
      hdr.appendChild(hint);
    }
    const closeBtn = document.createElement('button');
    closeBtn.textContent = 'X Close';
    closeBtn.style.cssText = 'background:rgba(255,255,255,0.15);border:1px solid rgba(255,255,255,0.3);color:#fff;border-radius:6px;padding:7px 14px;font-size:13px;font-weight:600;cursor:pointer;white-space:nowrap;';
    closeBtn.addEventListener('click', () => { overlay.remove(); try { URL.revokeObjectURL(pdfResult.blobUrl); } catch(e){} });
    ...
    if (isIOS) {
      ...
      body.style.cssText = 'flex:1;display:flex;align-items:center;justify-content:center;padding:32px;background:#f5f3ec;';
      const card = document.createElement('div');
      card.style.cssText = 'text-align:center;max-width:320px;';
      card.innerHTML = '<p style="font-size:48px;margin:0 0 16px;">&#128196;</p>' +
        '<p style="font-weight:600;font-size:16px;margin:0 0 8px;color:#1a1a1a;">PDF ready</p>' +
        '<p style="color:#7a7565;font-size:13px;margin:0 0 24px;">Tap below to open, then Share &#8594; <strong>Save to Files</strong>.</p>';
      const openLink = document.createElement('a');
      openLink.href = pdfResult.blobUrl;
      openLink.target = '_blank';
      openLink.rel = 'noopener';
      openLink.style.cssText = 'display:block;background:#f5d116;color:#0e4a2a;font-weight:700;font-size:15px;padding:14px;border-radius:8px;text-decoration:none;';
      openLink.textContent = 'Open PDF';
      card.appendChild(openLink);
      body.appendChild(card);
      overlay.appendChild(body);
    } else {
      // Set iframe.src via DOM property, never via innerHTML or template literal
      const iframe = document.createElement('iframe');
      iframe.title = 'PDF Preview';
      iframe.style.cssText = 'flex:1;border:none;width:100%;display:block;background:#525659;';
