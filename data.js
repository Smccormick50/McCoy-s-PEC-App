...
function bindViewEvents(p) {
  ...
  document.getElementById('btnPrint').addEventListener('click', async (e) => {
      ...
      overlay.innerHTML = `
        ...
        <div style="flex:1;display:flex;align-items:center;justify-content:center;padding:32px;background:#f5f3ec;">
          <div style="text-align:center;max-width:320px;">
            ...
            <p style="color:#7a7565;font-size:13px;margin:0 0 24px;">Tap below to open the PDF, then tap the Share icon → <strong>Save to Files</strong>.</p>
            <a href="${pdfResult.dataUri}" target="_blank"
               style="display:block;background:#f5d116;color:#0e4a2a;font-weight:700;font-size:15px;padding:14px;border-radius:8px;text-decoration:none;">
              Open PDF ↗
            </a>
          </div>
        </div>`;
    } else {
      // Desktop: embed via data URI — Chrome renders data:application/pdf in iframes
      // using its built-in PDF viewer (the ⬇ button in that viewer saves the file).
      // data: URIs avoid blob URLs and URL.createObjectURL entirely.
      overlay.innerHTML = `
        <div style="background:#0e4a2a;padding:12px 16px;display:flex;align-items:center;gap:12px;flex-shrink:0;">
          ...
          <span style="color:rgba(255,255,255,0.65);font-size:12px;">Use the ⬇ button inside the PDF to save</span>
          ${closeBtn}
        </div>
        <iframe src="${pdfResult.dataUri}"
    ...
    document.body.appendChild(overlay);
    document.getElementById('pdfClose').addEventListener('click', () => overlay.remove());
  });
