...
function scheduleSave(project) {
  ...
  saveTimer = setTimeout(() => {
    touch(project);
    DB.put(project).catch(err => { console.error('Autosave failed:', err); toast("Couldn't save changes"); });
  }, 400);
}
...
function storageErrorTemplate() {
  ...
  <main class="list-main">
    <div class="empty-state">
      <div class="empty-mark" style="border-color:var(--tape-dark); color:var(--tape-dark);">!</div>
...
async function handleFiles(fileList) {
  ...
  for (const file of files) {
    if (file.size > MAX_FILE_BYTES && !oversizeWarned) {
      toast(`Heads up — "${file.name}" is large and may be slow to load`);
      oversizeWarned = true;
    }
